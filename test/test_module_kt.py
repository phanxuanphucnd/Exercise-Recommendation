...
def test_train():
    ...
    group = group_by_user_id(data_df=data_df, mode='kt')
    train_group, test_group = train_test_split(data_df=data_df, pct=0.1, mode='kt')
    ...
    train_dataset = AKTDataset(
        train_group, n_question=n_question, max_seq=max_seq)
